    ...
    if len(activeSet) == 0:
        ...
        if explicitA:
            if len(np.array([R_I]).flatten()) == 1:
                p = scipy.linalg.solve(np.array(R_I).reshape(-1,1), 
                                       np.matmul(R[:,activeSet].transpose(), 
                                                 R[:,newIndex]), 
                                       transposed = True, 
                                       lower = False)
            else:
                p = scipy.linalg.solve(R_I, np.matmul(R[:,activeSet].transpose(), 
                                                      R[:,newIndex]), 
                                       transposed = True, 
                                       lower = False)
...
def lasso(R, s, sr, maxIters, epsilon):
    ...
    for j in range(0, len(newIndices)):
        iter = iter + 1
        R_I, flag = updateChol(R_I, N, W, R, 1, 
                               activeSet,
                               newIndices[j],
                               zeroTol)
    ...
    while done == 0:
        ...
        if len(np.array([R_I]).flatten()) == 1: 
            z = scipy.linalg.solve(R_I.reshape([-1,1]), 
                                   np.sign(c[np.array(activeSet).flatten()].reshape(-1,1)),
                                   transposed = True, 
                                   lower = False)
        else:
            z = scipy.linalg.solve(R_I, 
                                   np.sign(c[np.array(activeSet).flatten()].reshape(-1,1)),
                                   transposed = True, 
                                   lower = False)
        ...
        if len(np.array([R_I]).flatten()) == 1:
            dx[np.array(activeSet).flatten()] = scipy.linalg.solve(R_I.reshape([-1,1]), 
                                                                   z,
                                                                   transposed = False, 
                                                                   lower = False)
        else:
            dx[np.array(activeSet).flatten()] = scipy.linalg.solve(R_I, 
                                                                   z,
                                                                   transposed = False, 
                                                                   lower = False).flatten()
        
        
        v = np.matmul(R[:,np.array(activeSet).flatten()], 
...
def sparsEDA(signalIn,sr,
    ...
    while cutE < Ns:
        ...
        l_SCR = np.matmul(R[:,6:], beta[6:,:]) 
        SCR[cutS:cutS + (jump*20*sr)] = l_SCR[:(jump*20*sr)]
        ...
        list_MSE.append(np.mean(remAout[0:jump*sr*20]))
    ...
    list_dr = []
    while cutE < Ns:
        l_driver = driver[cutS:cutE]
        freq = len(np.argwhere(l_driver>0).flatten()) / (N/sr)
        list_dr.append(freq)
        cutS+=N 
        cutE+=N
        
    #plt.figure(figsize = (20,4), dpi = 100) 
    #plt.plot(R_t[7000: 9000], color = 'blue') 
    #plt.show()
    #plt.clf()
    
    return driver, SCL, SCR, list_MSE, list_dr
